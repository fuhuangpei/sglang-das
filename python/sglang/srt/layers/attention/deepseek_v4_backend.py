...
class DeepseekV4AttnBackend(
    ...
    def _forward_bf16_sparse_mla(
        ...
        *,
        ...
        assert swa_k_cache_bf16.shape[-1] == self.head_dim, (
            f"{swa_k_cache_bf16.shape=}, {self.head_dim=}"
        )
        ...
        if extra_k_cache_bf16 is not None:
            ...
            assert extra_k_cache_bf16.shape[-1] == self.head_dim, (
                f"{extra_k_cache_bf16.shape=}, {self.head_dim=}"
            )
    ...
    def forward(
        ...
        **_,
        ...
        if isinstance(core_attn_metadata, DSV4AttnMetadata):
            ...
            if use_bf16_sparse_attn:
                assert swa_k_cache.ndim == 4, f"{swa_k_cache.shape=}"
            ...
            if extra_k_cache is not None:
                ...
                if use_bf16_sparse_attn:
                    assert extra_k_cache.ndim == 4, f"{extra_k_cache.shape=}"
